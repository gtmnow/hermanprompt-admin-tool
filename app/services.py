...
def tenant_onboarding_status_orm_compatible(db: Session) -> bool:
...
def _build_onboarding_status_snapshot(
    ...
    *,
    ...
    onboarding = persisted or SimpleNamespace(
        tenant_id=tenant_id,
        updated_at=datetime.now(timezone.utc),
        first_login_detected=False,
        first_transform_detected=False,
        first_score_detected=False,
    )
    ...
    if table_exists(db, "rag_documents"):
        try:
...
def refresh_onboarding_state(db: Session, tenant_id: str) -> TenantOnboardingStatus:
    if not tenant_onboarding_status_orm_compatible(db):
